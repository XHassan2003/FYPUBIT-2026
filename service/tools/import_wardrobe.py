...
APP_ROOT = next(
    (p for p in Path(__file__).resolve().parents if (p / "data" / "wardrobe.ts").exists()),
    None,
)
...
def read_vocabulary() -> dict:
    ...
    """
    wardrobe = (APP_ROOT / "data" / "wardrobe.ts").read_text(encoding="utf-8")
...
def render(items: list[dict]) -> str:
    ...
    lines = [
        ...
        "//",
        "// Your own clothes, photographed by you. Not currently wired into the",
        "// app — wardrobes are per-account now, and a file bundled into the app",
        "// ships to every account. See the note at the top of this file's",
        "// hand-written placeholder version for the reasoning.",
        "//",
...
def main() -> int:
    ...
    if APP_ROOT is None:
        print("X  could not locate the app root (looked for data/wardrobe.ts)")
        return 1
